    ...
    class Mixpanel
      ...
      def init_javascript(location)
        init_location(location) do
          ...
          var mixpanel_tracking_opt = {}; if ((document.cookie.replace(/(?:(?:^|.*;\s*)performance_cookie\s*\=\s*([^;]*).*$)|^.*$/, "$1") != "true") && (document.cookie.indexOf("performance_cookie") != -1)) { mixpanel_tracking_opt = {opt_out_tracking_by_default: true} }
          mixpanel.init('#{options[:key]}', mixpanel_tracking_opt);</script><!-- end Mixpanel -->
          HTML
          js
        end
      end
